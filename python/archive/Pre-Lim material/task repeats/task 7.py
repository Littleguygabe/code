...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            ###
            choice = ''
            if len(self.prevMoves)>0:
                print(f'you have {len(self.prevMoves)} moves to undo')
                choice = input('Do you want to undo a move? (Y/N)').upper()
                if choice == 'Y':
                    self.undoMove()
            ###
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):
                ###
                undoCell = previousMove(Row,Column,CurrentCell.GetSymbol())
                undoCell.setSymbolsNotAllowed(CurrentCell.getSymbolsNotAllowed())
                self.prevMoves.append(undoCell)
                ###
    ...
    def undoMove(self):
        cell = self.prevMoves.pop()
        Row = cell.Row
        Column = cell.Col
        Index = (self.__GridSize - Row) * self.__GridSize + Column - 1
        newCell = Cell()
        newCell.setSymbolsNotAllowed(cell.getSymbolsNotAllowed())
        newCell.ChangeSymbolInCell(cell.GetSymbol())
        self.__Grid[Index]=newCell
        print()
        self.DisplayPuzzle()
        print()
...
class previousMove(Cell):
    ...
    def setSymbolNotAllowed(self,symbolsNotAllowed):
        self.__SymbolsNotAllowed = symbolsNotAllowed
