        ...
        while not Finished:
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):
                ...
                if AmountToAddToScore > 0:
                    ...
                    ###
                    QReshuffle = input('Do you want to reshuffle the blocked cells in the grid? (y/n)').lower()[0]
                    if QReshuffle == 'y':
                        self.reshuffleblocked()
...
###
    def reshuffleblocked(self):
        blockedIndex = []
        for i in range(0,len(self.__Grid)-1):
            if self.__Grid[i].GetSymbol() == '@':
                blockedIndex.append(i)
                self.__Grid[i] = Cell()

        newBlocked = 0
        while newBlocked<len(blockedIndex):
            newIndex = random.randint(0,len(self.__Grid)-1)
            print(newBlocked)
            if self.__Grid[newIndex].GetSymbol() == '-' and newIndex not in blockedIndex:
                self.__Grid[newIndex] = BlockedCell()
                newBlocked+=1
