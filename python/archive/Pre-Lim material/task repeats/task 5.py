...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            ###
            print(f'You have {self.__SymbolsLeft} symbols left');deleteQ = input('do you want to delete a symbol?(Y/N)')
            delete = True if deleteQ.upper()=='Y' else False
            ###
            ...
            while not Valid:
                try:
                    ...
                    pass
            ###
            if delete:self.deleteCell(Row,Column)
            else:
            ###
    ...
    def deleteCell(self,row,col):
        if self.__GetCell(row,col).GetSymbol()!= '@' and self.__GetCell(row,col).GetSymbol()!='-' and self.CheckforMatchWithPattern(row,col)==0:self.__GetCell(row,col).ChangeSymbolInCell('');self.__SymbolsLeft+=1
        else:print('Invalid cell location')
    ###
