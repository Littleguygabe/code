            ...
            for StartColumn in range(Column - 2, Column + 1):
                try:
                    ...
                    for P in self.__AllowedPatterns:
                        ...
                        ###
                        pattern = P.GetPatternSequence()
                        for i in range(1,5):
                            if i == 1:
                                patterntemp = pattern
                            elif i == 2:
                                patterntemp = pattern[6:8]+pattern[:6]+pattern[-1]
                            elif i == 3:
                                patterntemp = pattern[4:8]+pattern[:4]+pattern[-1]
                            elif i == 4:
                                patterntemp = pattern[2:9]+pattern[:2]

                        if P.MatchesPattern(PatternString, CurrentSymbol,patterntemp): #passes in a 9 element long array that contains the pattern of a 3x3 grid
...
class Pattern():
    ...
    def MatchesPattern(self, PatternString, SymbolPlaced,PatternToMatch):
        print(PatternToMatch)
        if SymbolPlaced != self.__Symbol:
            return False
        for Count in range(0, len(PatternToMatch)):
            try:
                if PatternToMatch[Count] == self.__Symbol and PatternString[Count] != self.__Symbol:
                    return False
