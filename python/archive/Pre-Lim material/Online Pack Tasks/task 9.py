...
class Puzzle():
    def __init__(self, *args):
        if len(args) == 1:
            ...
            ###
            nNeededDoubles = 5 ## set the number of required doubles within the game
            nDoubles = 0 ## the current number of double point celkls
            while nDoubles<nNeededDoubles:
                index = random.randint(0,len(self.__Grid)-1) ## generate a random index for the cell
                if self.__Grid[index].GetSymbol() == '-':   ## if the randomly picked cell is empty 
                    self.__Grid[index] = DoublePointCell()  ## then it can be assigned to a double cell
                    nDoubles+=1
                ...
                try:
                    ...
                    for P in self.__AllowedPatterns:
                        ...
                        if P.MatchesPattern(PatternString, CurrentSymbol): #passes in a 9 element long array that contains the pattern of a 3x3 grid
                            ...
                            print('pattern match')
                            multiplier = 1
                            for x in range(0,3): ## Loops from 0 to 2, to reduce the number of lines of code needed to check 3x3
                                for y in range(-2,1): ## Loops from -2 to 0, to allow the program to go from top to bottom on the 3x3
                                    if self.__GetCell(StartRow+x, StartColumn+y).isDouble(): ## calls the modified function to check if the cell is double
                                        multiplier = 2
