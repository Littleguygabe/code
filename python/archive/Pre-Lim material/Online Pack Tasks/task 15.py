                ...
                try:
                    ...
                    for P in self.__AllowedPatterns:
                        ...
                        for i in range(90,360,90):
                            if P.matchesRotatedPattern(i,PatternString,CurrentSymbol):
                                self.__GetCell(StartRow, StartColumn).AddToNotAllowedSymbols(CurrentSymbol) #top left
                                self.__GetCell(StartRow, StartColumn + 1).AddToNotAllowedSymbols(CurrentSymbol) #top center
                                self.__GetCell(StartRow, StartColumn + 2).AddToNotAllowedSymbols(CurrentSymbol) #top right
                                self.__GetCell(StartRow - 1, StartColumn + 2).AddToNotAllowedSymbols(CurrentSymbol) #right center
...
class Pattern():
    ...
    ###
    def matchesRotatedPattern(self,rotation,patternString,CurrentSymbol):
        if CurrentSymbol!=self.__Symbol:
            return False
        if rotation == 90:
            RotatedString = self.__PatternSequence[6:8]+self.__PatternSequence[:6]+self.__PatternSequence[-1]
        
        elif rotation == 180:
            RotatedString = self.__PatternSequence[4:8]+self.__PatternSequence[:4]+self.__PatternSequence[-1]

        elif rotation == 270:
            RotatedString = self.__PatternSequence[2:]+self.__PatternSequence[:2]

        for i in range(len(self.__PatternSequence)-1):
            if RotatedString[i] == self.__Symbol and patternString[i] != self.__Symbol:
                return False
        return True
