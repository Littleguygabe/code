...
class Puzzle():
    ...
    def savePuzzle(self):
        ...
        name = str(input('Enter the save name: '))
        f = open(f'{name}.txt','w')
        ...
        #symbol,associated pattern
        for i in range(0,len(self.__AllowedSymbols)):
            f.write(self.__AllowedSymbols[i]+','+self.__AllowedPatterns[i].GetPatternSequence()+'\n')

        #number of rows and columns
        f.write(str(self.__GridSize)+'\n')
        #rows an columns
            #current cell value, blocked values from going into that cell
        for cell in self.__Grid:
            string = ','.join(cell.getNotAllowed())

            if cell.GetSymbol() == '-':
                writeSymbol = ''
            else:
                writeSymbol = cell.GetSymbol()
            f.write(writeSymbol+','+string+'\n')
        #current score
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            Valid = False
            while not Valid:
                save = str(input('Do you want to save your game?'))
                save=save.upper()
                if save == 'Y' or save == 'N':
                    Valid = True
                    if save == 'Y':
