...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            ###
            undoing = True
            while undoing and len(self.__UndoMoves)>0:
                print(f'you have {len(self.__UndoMoves)} move(s) to undo')
                undo = input('Do you want to undo a move? (Y/N)')
                if undo.upper() == 'Y':
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):
                ...
                ###
                Move = previousMove(Row,Column,CurrentCell)
                self.__UndoMoves.append(Move)
                ###
...
###
    def undoPreviousMove(self):
        undoMove = self.__UndoMoves.pop()
        gridCell = self.__Grid[(self.__GridSize- undoMove.GetRow())*self.__GridSize+undoMove.GetColumn()-1]
        gridCell.ChangeSymbolInCell(undoMove.GetSymbol())
        gridCell.SetNotAllowedSymbols(undoMove.GetNotAllowedSymbolsList())
###
...
###
    ...
    def SetNotAllowedSymbols(self,NotAllowedSymbolsFromPreviousMove):
        self.__SymbolsNotAllowed = NotAllowedSymbolsFromPreviousMove

    def GetNotAllowedSymbolsList(self):
...
class previousMove(Cell):
    def __init__(self,Row,Column,cell):
        super(previousMove,self).__init__()
        self.__Row = Row
        self.__Column = Column
        self.ChangeSymbolInCell(cell.GetSymbol()) # need to change the cell class to facilitate the new functions
        self.SetNotAllowedSymbols(cell.GetNotAllowedSymbolsList()) # same as line above
