...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            Valid = False
            ###
            if self.Lever and not self.LeverUsed: 
                choice = input('Do you want to use your lever? (Y/N)').upper()
                if choice == 'Y': usingLever=True
            ###
            while not Valid:
                try:
            ...
            ###
            if usingLever:
                if self.__Grid[(self.__GridSize - Row) * self.__GridSize + Column - 1].GetSymbol()=='@':self.__Grid[(self.__GridSize - Row) * self.__GridSize + Column - 1] = Cell();self.Lever,self.LeverUsed = False,True
                else:
                    print('That cell is not a blocked cell')
            else:
            ###
