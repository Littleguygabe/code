...
###
    def checkForError1(self):
        error = False
        for cell in self.__Grid:
            if cell.containsSymbolsNotAllowedList() and cell.GetSymbol() != '@':
                error = True
            
        if error and self.__Score == 0:
            print('Error 1 Found')

    def checkForError2(self):
        count = 0
        for cell in self.__Grid:
            if cell.containsSymbolsNotAllowedList():
                count+=1
        
        if (self.__Score/10)*9>count:
...
###
    def containsSymbolsNotAllowedList(self):
        emptyString = False
        if len(self.__SymbolsNotAllowed)>0 and '' not in self.__SymbolsNotAllowed or self._Symbol == '@':
            return True
        return False
###
