...
class Puzzle():
    def __init__(self, *args):
        if len(args) == 1:
            ...
            ###
            dif = int(input('Choose difficulty: 1 - easy, 2 - medium, 3 - hard: '))
            for Count in range(1, self.__GridSize * self.__GridSize + 1):
                if random.randrange(1, 101) < (100-10 - (10*(dif-1))): # ~90% of grid squares should be empty cells ### change for the question
                    C = Cell()
                else:
                    C = BlockedCell()
