    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):
                ...
                ###
                EnoughSymbols = self.GetSpaceLeft()
            
            if not EnoughSymbols:
                print('There is not enough symbols to complete another pattern')
                print(f'{self.__SymbolsLeft} points will be deducted from your score')
...
###
    def GetSpaceLeft(self):
        availablePatternSpaces = []
        enoughSymbols4pattern = False
        for checkSymbol in self.__AllowedSymbols:
            for StartRow in range(self.__GridSize,2,-1):
                for StartColumn in range(1,self.__GridSize-1):
                    ...
                    availablePatternSpaces.append(PatternString)
            for checkPattern in availablePatternSpaces:
                if enoughSymbols4pattern:
                    break
                for P in self.__AllowedPatterns:
                    matchingChars = 0
                    chars4match = 0
                    AllowedPattern = P.GetPatternSequence()
                    if not P.MatchesPattern(checkPattern,checkSymbol):
                        for i in range(len(checkPattern)):
                            if AllowedPattern[i] == checkPattern[i] and checkPattern[i] == checkSymbol:
                                matchingChars+=1
                            if AllowedPattern[i]!='*':
                                chars4match+=1
                        if chars4match-matchingChars<=self.__SymbolsLeft:
                            enoughSymbols4pattern=True
                            break
        return enoughSymbols4pattern
###
