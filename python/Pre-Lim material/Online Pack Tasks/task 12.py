...
class Puzzle():
    ...
    def swampThisCell(self):
        valid = False
        while not valid:
            i = random.randint(0,len(self.__Grid)-1)
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            ###
            if self.nSwampTurns == 0 and not self.swampInitiated:
                randDecider = random.randint(1,1)
                if randDecider == 1:
                    self.nswamps = random.randint(1,4)
                    self.nSwampTurns = random.randint(2,4)
                    self.swampInitiated = True
                    print(f'A swamp was triggered, you have {self.nSwampTurns} turns before a swamp of {self.nswamps} cells appears')

            if self.nSwampTurns>0:
                print(f'{self.nSwampTurns} turns before the swamp appears')

            if self.nSwampTurns==0 and self.swampInitiated:
                for i in range(self.nswamps):
                    self.swampThisCell()
            
                print('Swamping has occurred')
                self.DisplayPuzzle()
            ###
            Row = -1
            Valid = False
