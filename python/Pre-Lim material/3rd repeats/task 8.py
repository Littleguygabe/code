...
class Puzzle():
    ...
    def reShuffleBlockedCells(self):
        prevPos = []
        for i in range(len(self.__Grid)-1):
            if self.__Grid[i].GetSymbol() == '@':
                prevPos.append(i)
                self.__Grid[i]=Cell()
        for i in range(len(prevPos)):
            Index = (self.__GridSize - (random.randint(0,self.__GridSize))) * self.__GridSize + (random.randint(0,self.__GridSize)) - 1
            while Index in prevPos:
                Index = (self.__GridSize - (random.randint(0,self.__GridSize))) * self.__GridSize + (random.randint(0,self.__GridSize)) - 1
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            ###
            if self.reshuffle:
                choice = input('Do you want to reshuffle the blocked cells? (Y/N): ').upper()
                if choice == 'Y':
