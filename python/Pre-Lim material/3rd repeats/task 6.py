...
class Puzzle():
    ...
    def save(self):
        with open(input('Enter the file name to save to: ')+'.txt','w') as f:
            f.write(f'{str(len(self.__AllowedSymbols))}\n')
            f.write(f'{'\n'.join(self.__AllowedSymbols)}\n')
            f.write(str(len(self.__AllowedPatterns)))
            for i in range(0,len(self.__AllowedPatterns)):
                f.write(f'{self.__AllowedSymbols[i]},{self.__AllowedPatterns[i].GetPatternSequence()}\n')
            f.write(f'{self.__GridSize}\n')
            for cell in self.__Grid:
                f.write(f'{cell.getSaveSymbol()},{','.join(cell.getSymbolsNotAllowed())}\n')
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            ###
            choice = input('Do you want to save the game? (Y/N) ').upper()
            if choice == 'Y':
                self.save()
            Row = -1
            Valid = False
...
class Cell():
    ...
    def UpdateCell(self):
        ...
        ###
    def getSaveSymbol(self):
        return self._Symbol
    
    def getSymbolsNotAllowed(self):
        if len(self.__SymbolsNotAllowed)>0 and self.__SymbolsNotAllowed[0] == '':
            del self.__SymbolsNotAllowed[0]
