...
class Puzzle():
    ...
    def undoPreviousMove(self):
        uc = self.PMstack.pop()
        Row,Column = uc.getRowCol()

        newCell = Cell()
        newCell.ChangeSymbolInCell(uc.GetSymbol())
        newCell.setSymbolsNA(uc.getNASymbols())

        self.__Grid[(self.__GridSize - Row) * self.__GridSize + Column - 1] = newCell
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            ###
            if len(self.PMstack)>0:
                print(f'You have {len(self.PMstack)} moves to undo')
                choice = input('Do you want to undo a previous move? (Y/N)').upper()
                if choice == 'Y':
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):
                ...
                ###
                self.PMstack.append(previousMove(Row,Column,CurrentCell.GetSymbol(),CurrentCell.getNASymbols()))
...
class Cell():
    ...
    def getNASymbols(self):
        return self.__SymbolsNotAllowed

    def setSymbolsNA(self,list):
        self.__SymbolsNotAllowed = list

###
class previousMove(Cell):
    def __init__(self,Row,Col,symbol,symbolsNA):
        super(previousMove,self).__init__()
        self.ColPos = Col
        self.RowPos = Row
        self.__SymbolsNotAllowed = symbolsNA
        self._Symbol = symbol
