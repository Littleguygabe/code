...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if self.getSpaceLeftOnGrid():
                Finished = True
                print()
                self.DisplayPuzzle()
                print()
                print('No more patterns can be created with the amount of symbols left')
                return self.__Score-self.__SymbolsLeft
        ...
        for checkSymbol in self.__AllowedSymbols:
            for StartRow in range(self.__GridSize,2,-1):
                for StartColumn in range(1,self.__GridSize-1):
                    ...
                    possiblePatterns.append(PatternString)

            for pattern in possiblePatterns:
                for P in self.__AllowedPatterns:
                    currMatch = 0
                    totalMatch = 0
                    checkPattern = P.GetPatternSequence()
                    for i in range(0,len(pattern)):
                        if pattern[i] == checkPattern[i] and pattern[i] == checkSymbol:
                            currMatch+=1
                        if checkPattern[i]!='*':
                            totalMatch+=1
                    if totalMatch-currMatch<=self.__SymbolsLeft:
                        enoughSymbols = True
                        break
